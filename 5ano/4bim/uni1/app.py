...
@st.cache_data
def create_fraction_bar_image(n_list, d_list, colors):
    img_width = 800
    bar_height = 80
    padding = 20
    vertical_gap = 120 # Aumentado o espaço entre as barras
    ...
    for i, (n, d, color) in enumerate(zip(n_list, d_list, colors)):
        ...
        # Desenha as linhas divisórias em PRETO para contraste
        for j in range(1, d):
...
op_select = st.sidebar.radio("Selecione a Operação:", ('Soma', 'Subtração'), key='op_sim', on_change=lambda: setattr(st.session_state, 'step', 'INPUT_MMC'))
st.sidebar.markdown("---")
n1 = st.sidebar.number_input("Numerador 1", min_value=1, value=1, key="n1_key", on_change=lambda: setattr(st.session_state, 'step', 'INPUT_MMC'))
d1 = st.sidebar.number_input("Denominador 1", min_value=2, value=4, key="d1_key", on_change=lambda: setattr(st.session_state, 'step', 'INPUT_MMC'))
st.sidebar.markdown("---")
n2 = st.sidebar.number_input("Numerador 2", min_value=1, value=1, key="n2_key", on_change=lambda: setattr(st.session_state, 'step', 'INPUT_MMC'))
d2 = st.sidebar.number_input("Denominador 2", min_value=2, value=6, key="d2_key", on_change=lambda: setattr(st.session_state, 'step', 'INPUT_MMC'))
...
with text_col1:
    st.markdown(f"<p style='font-size: 4vw; font-weight: bold; margin-top: 3vh;'>{n1}/{d1}</p>", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size: 4vw; font-weight: bold; margin-top: 3vh;'>{op_select[0]}</p>", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size: 4vw; font-weight: bold; margin-top: 3vh;'>{n2}/{d2}</p>", unsafe_allow_html=True)

# ETAPA 2: TESTE DO MMC
st.header("ETAPA 2: Encontrando o Denominador Comum")
mmc_guess = st.number_input(f"Para somar, precisamos de pedaços iguais. Qual é o denominador comum (MMC) para {d1} e {d2}?", min_value=1, key="mmc_input")

if st.button("Verificar MMC", type="primary"):
    if mmc_guess == correct_mmc:
        st.session_state.mmc_correct = True
        st.success(f"**Exato!** O denominador comum é **{correct_mmc}**.")
    else:
        st.session_state.mmc_correct = False
        st.error(f"Incorreto. O MMC de {d1} e {d2} não é {mmc_guess}. Tente de novo! Dica: procure o primeiro número que aparece na tabuada dos dois.")

# ETAPAS SEGUINTES SÓ APARECEM SE O MMC ESTIVER CORRETO
if st.session_state.mmc_correct:
    n1_equiv = n1 * (correct_mmc // d1)
    n2_equiv = n2 * (correct_mmc // d2)

    st.header("ETAPA 3: Transformando as Frações")
    st.info("Agora que sabemos o denominador comum, veja como as frações originais se transformam:")
    ...
    with img_col2:
        ...
        st.image(equiv_img)
    with text_col2:
        st.markdown(f"<p style='font-size: 2vw; margin-top: 1vh;'>Original: <strong>{n1}/{d1}</strong></p><p style='font-size: 4vw; font-weight: bold;'>{n1_equiv}/{correct_mmc}</p>", unsafe_allow_html=True)
        st.markdown(f"<p style='font-size: 2vw; margin-top: 6vh;'>Original: <strong>{n2}/{d2}</strong></p><p style='font-size: 4vw; font-weight: bold;'>{n2_equiv}/{correct_mmc}</p>", unsafe_allow_html=True)
    ...
    > * Para a segunda ($$\\frac{{{n2}}}{{{d2}}}$$): fazemos o mesmo ($${correct_mmc} \\div {d2} = {correct_mmc//d2}$$), e então ($${n2} \\times {correct_mmc//d2} = {n2_equiv}$$).
    """)

    st.header("ETAPA 4: Qual é a Resposta Final?")
    st.warning(f"Calcule o resultado de $$\\frac{{{n1_equiv}}}{{{correct_mmc}}} {op_select[0]} \\frac{{{n2_equiv}}}{{{correct_mmc}}}$$ e digite sua resposta.")
...
if st.session_state.step == "SHOW_FINAL" and st.session_state.mmc_correct:
    if op_select == 'Soma':
    ...
    with img_col3:
        final_img = create_fraction_bar_image([n1_equiv, n2_equiv, correct_n_final], [correct_mmc, correct_mmc, correct_mmc], ['#4A90E2', '#50E3C2', '#E34A7F'])
        st.image(final_img)
    with text_col3:
         st.markdown(f"""
         <div style='margin-top: 1vh;'>
             <p style='font-size: 4vw; font-weight: bold;'>{n1_equiv}/{correct_mmc}</p>
             <p style='font-size: 4vw; font-weight: bold;'>{op_select[0]}</p>
             <p style='font-size: 4vw; font-weight: bold;'>{n2_equiv}/{correct_mmc}</p>
             <p style='font-size: 4vw; font-weight: bold;'>=</p>
             <p style='font-size: 4vw; font-weight: bold; color: #E34A7F;'>{correct_n_final}/{correct_mmc}</p>
         </div>
